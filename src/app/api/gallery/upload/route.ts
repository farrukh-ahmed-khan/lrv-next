// /app/api/librarys/upload/route.ts
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const libraryId = Array.isArray(fields.libraryId)
      ? fields.libraryId[0]
      : fields.libraryId;
    ...
    //   const s3Key = `librarys/${libraryId}/${Date.now()}_${image.originalFilename}`;
    ...
    library.images.push(...uploadedUrls);
    await library.save();

    return NextResponse.json(
      { message: "Images uploaded", library },
      { status: 200 }
    );
  } catch (error: any) {
