...
import { addLrvlaw, deleteLrvlaw, getLrvlaws, updateLrvlaw } from "@/lib/LrvLawApi/api";
...
const LrvLaw = () => {
    ...
    const [lrvlawData, setLrvlawData] = useState<any[]>([]);
    ...
    const getAllLrvlaws = async () => {
        if (!token) return toast.error("Please Login");
        try {
            const res = await getLrvlaws(token);
            const fetchedData = res.lrvlaw.map((data: Lrvlaws, index: number) => ({
                key: data._id,
                id: data._id,
                title: data.title,
                file: data.fileUrl
            }));
            setLrvlawData(fetchedData);
        } catch (error) {
            console.error("Fetch Error:", error);
        }
    };

    const handleAddLrvlaws = async (values: any) => {
        ...
        try {
            setLoading(true);
            const res = await addLrvlaw(formData, token);
    ...
    const handleUpdate = async (values: any) => {
        ...
        try {
            setLoading(true);
            await updateLrvlaw(formData, token);
                        ...
                        <div className="row">
                            <div className="col-md-12">
                                ...
                                <div className="store-table-wrap active-table">
                                    <Table columns={columns} dataSource={lrvlawData} />
                                </div>
                                ...
                                <Modal
                                    ...
                                    footer={null}
                                >
                                    <Form layout="vertical" form={form} onFinish={handleAddLrvlaws}>
                                        <Form.Item
                                            label="Title Name"
                                            name="titlename"
